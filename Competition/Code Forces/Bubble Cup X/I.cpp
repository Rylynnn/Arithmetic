...
void solve()
{
    ...
    doit(lc);ans[q[1].id]=now;doit(lc);
    for(LL i=2;i<=tot;i++)
    {                                               
        work(q[i].x,q[i-1].x);
        work(q[i].y,q[i-1].y);
        int lc=lca(q[i].x,q[i].y);
        doit(lc);ans[q[i].id]=now;
        doit(lc);
    }
}
/*
...
//  freopen("my.txt","w",stdout);
    ...
    for(int i=1;i<=n;i++)NUM[i]=fav[i]=read();
    sort(NUM+1,NUM+1+n);NUM[0]=-1;
    for(int i=1;i<=n;i++)if(NUM[i]!=NUM[i-1])HAS[++TOT]=NUM[i];
    for(int i=1;i<=n;i++)fav[i]=find(fav[i]);
    //for(int i=1;i<=n;i++)printf("%d ",fav[i]);
    for(int i=1;i<n;i++)
    {
        int x=read(),y=read();
