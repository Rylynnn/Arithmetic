...
double cross_product(Point a, Point b)//²æ³Ë
{
...
int main()
{
    while(scanf("%lf%lf%lf",&m.x,&m.y,&r)!=EOF){
        if(r<=0){
            continue;
        }
        scanf("%d",&n);
        ...
        for(int i=0;i<k;i++){
            cnt=1;//已经从第一个点加入
            for(int j=0;j<k;j++){
                if(i!=j&&cross_product(m,p[i],p[j])>=0){//圆心到直径穿过的点和将要判断的点，两者和圆心组成的向量的叉积，大于等于0，表示sin值在0~PI之间故在直径的一侧
                    cnt++;
                }
            }
            ans=max(ans,cnt);
