...
struct Node {
    ...
    int size,sum,lmx,rmx,mx;//sum表示区间和，lmx表示左起最大子段和，rmx表示右起最大子段和，mx表示区间最大子段和
    int lazy;//lazy^1表示是否翻转，lazy^2表示是否修改为某一值
    ...
    void modify() {//确认当前区间统一修改为key值
        lazy=2;
        sum=key*size;
        lmx=rmx=mx=(key<0?key:sum);//如果key<0，则最大值为key，否则为sum
    }

    void pushUp() {//将子结点的值跟新到当前结点
        size=lson->size + 1 + rson->size;
        sum =lson->sum  +key+ rson->sum ;
        lmx=max(lson->lmx,lson->sum+key+max(0,rson->lmx));
        rmx=max(rson->rmx,rson->sum+key+max(0,lson->rmx));
        mx =max( max(lson->mx,rson->mx) , max(0,lson->rmx)+key+max(0,rson->lmx) );
        lazy=0;
    }
    ...
    void pushDown() {
        ...
        if((lazy&2)==2) {
            lson->key=rson->key=key;
            lson->modify();//确认lson的修改
            rson->modify();//确认rson的修改
        }
        lazy=0;
    }
...
void inser(int i,int cnt) {//在第i个数后插入一个长度为cnt的序列num[1..cnt]
    Node *x,*y,*z;
    y=findNo( i, z=findNo(i+1) );//将第i+1个结点z旋转至根结点，将第i个结点y旋转至此时的根节点下（必在其左儿子），则要插入的序列在y结点的右子树中
    x=build(1,cnt);
    y->setRson(x);//将插入的序列子树挂在y结点的右儿子处
    y->pushUp();//由于y结点是z结点的儿子，所以需要先更新y结点
    z->pushUp();
}

void delet(int l,int r) {//删除区间[l,r]
    Node *x,*y,*z;
    y=findNo( l-1, z=findNo(r+1) );//将r的后一个结点z旋转至根结点，将l的前一个结点y旋转至此时的根节点下（必在其左儿子），则要删除的区间[l,r]在y结点的右子树中
...
void revers(int l,int r) {//翻转区间[l,r]
    Node *x,*y,*z;
    y=findNo( l-1, z=findNo(r+1) );//将r的后一个结点z旋转至根结点，将l的前一个结点y旋转至此时的根节点下（必在其左儿子），则要翻转的区间[l,r]在y结点的右子树中
    x=y->rson;//x结点子树为区间[l,r]
    x->reverse();//翻转区间[l,r];
    y->pushUp();//由于y结点是z结点的儿子，所以需要先更新y结点
    z->pushUp();
...
void makeSame(int l,int r) {//将区间[l,r]内的数都变成一个读入的数
    Node *x,*y,*z;
    y=findNo( l-1, z=findNo(r+1) );//将r的后一个结点z旋转至根结点，将l的前一个结点y旋转至此时的根节点下（必在其左儿子），则要翻转的区间[l,r]在y结点的右子树中
    x=y->rson;//x结点子树为区间[l,r]
    scanf("%d",&x->key);
...
int getSum(int l,int r) {//返回区间[l,r]的数的和
    Node *x,*y,*z;
    y=findNo( l-1, z=findNo(r+1) );//将r的后一个结点z旋转至根结点，将l的前一个结点y旋转至此时的根节点下（必在其左儿子），则要查询的区间[l,r]在y结点的右子树中
    x=y->rson;//x结点子树为区间[l,r]
    y->pushUp();//由于y结点是z结点的儿子，所以需要先更新y结点
    z->pushUp();
    return x->sum;
    ...
    while(T-->0) {
        ...
        while(q-->0) {
            ...
            if(ch=='K') {//MAKE-SAME
                ...
                scanf("%d%d",&sta,&cnt);
                revers(sta,sta+cnt-1);
            }
            else if(ch=='T') {//GET-SUM
                scanf("%d%d",&sta,&cnt);
                printf("%d\n",getSum(sta,sta+cnt-1));
            }
            else {//MAX-SUM
                printf("%d\n",root->mx);
            }
